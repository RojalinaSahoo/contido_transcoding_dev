...
def login_api(request):
    if not request.data:
        return Response({'message': "Please provide email and organisation name","data":None,"status":400})
    try:
        email = request.data['email']
        organisation = request.data['organisation']
    except Exception as e:
        print(e)
        return Response({'message': "Invalid parameter","data":None,"status":400}) 
    ...
    try:
        ...
        for user_data in user:
             ...
             project_title = user_data.project_title
        if user_data.username != email:
             return Response({'message': "Email id doesn't exists","data":None,"status":400})
        elif user_data.project_title != organisation:
             return Response({'message': "project title not associated with user","data":None,"status":400})
    except Exception as e:
        print(e)
        return Response({'message': "Your profile doesn't exist with us.","data":None,"status":400})
        #if user_data:
    current_time = int(datetime.datetime.now().timestamp())
    data_current_time=datetime.datetime.fromtimestamp(current_time).strftime("%Y-%m-%d %H:%M:%S")
    print("current time:",data_current_time)
    ...
    #    return Response({"Error":"Token is Expired"},status="400")
    if (current_time > new_token_created) and (current_time < new_expire_time):
        return Response({"message":"Token is already created. You can access api","data":None,"status":200})
    elif user_data.transcode_token:
        return Response({"message":"Use refresh token because one time token created already","data":None,"status":400})
    elif current_time > new_expire_time and user_data.api_token_recreate == False:
        ...
        try:
            ...
            print(e)
            return Response({"message":"Token is Expired","data":None,"status":400})
        user_data.transcode_token = compact_jws
        user_data.api_created_token_at = data_current_time
        user_data.api_expired_token_at = data_expirytime2
        #user.api_token_recreate = False
        ...
        try:
            ...
            return Response({"message":"Token is Expired","data":None,"status":400})
        print(message_received)
        return Response({"token":compact_jws,"message":"success","status":200})
...
def login_api_refresh(request):
    ...
    try:
        #email = request.data['email']
        #organisation = request.data['organisation']
        token_refresh = request.data['token']
    except Exception as e:
        print(e)
        return Response({'message': "Invalid parameter","data":None,"status":400})

    try:
        user = UserInfo.objects.filter(transcode_token=token_refresh)
        ...
        #elif user_data.project_title != organisation:
        #     return Response({'message': "project title not associated with user","data":None,"status":400})
        if user_data.transcode_token != token_refresh:
             return Response({'message': "token doesn't match","data":None,"status":400})
    ...
    #    return Response({"Error":"Token is Expired"},status="400")
    if (current_time > new_token_created) and (current_time < new_expire_time):
        return Response({"message":"Token is already exists. You can access api","data":None,"status":200})
    elif current_time > new_expire_time and api_token_recreate == False:
        ...
        try:
            ...
            return Response({"message":"Token is Expired","data":None,"status":400})
        print(message_received)
        return Response({"token":refresh_jws,"message":"success","status":200})
...
def is_token_required(function):
    def wrap(request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"message":"token not found.","data":None,"status":400})
    return wrap
